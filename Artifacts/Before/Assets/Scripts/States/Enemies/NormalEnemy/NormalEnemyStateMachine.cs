...
public class NormalEnemyStateMachine : StateMachine, IDamageable, IWeapon, ISquadMember, IHitReactable
{
    ...
    private void UpdateTint()
    {
        ...
        currentTint = Color.Lerp(zeroHealthColor, fullHealthColor, t);
        ApplyTint(currentTint);
    }

    public void ReactToHit(RaycastHit hit, Vector3 shotDirection, float damage)
    {
        if (rend == null || currentHealth <= 0f)
            return;

        if (hitReaction != null)
            StopCoroutine(hitReaction);
        hitReaction = StartCoroutine(HitReactionRoutine());
    }

    private IEnumerator HitReactionRoutine()
    {
        ApplyTint(Color.Lerp(currentTint, Color.white, 0.8f));
