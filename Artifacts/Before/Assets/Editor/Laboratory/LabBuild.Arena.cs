...
public static partial class LabBuild
{
    static void Arena()
    {
        ...
        for(int side=-1;side<=1;side+=2)
        {
            ...
            Box("Bridge guard",world,new Vector3(side*13,3.55f,6.5f),new Vector3(8,.85f,.14f),glass);
            Box("Bridge rail",world,new Vector3(side*13,4,6.5f),new Vector3(8,.08f,.16f),cyan,false);
        ...
        Sign("H E L I X   /   0 7",new Vector3(0,6.2f,19.62f),Quaternion.Euler(0,180,0),14,new Color(.08f,.45f,.55f));
        Sign("CONTAINMENT RESEARCH",new Vector3(0,5.15f,19.6f),Quaternion.Euler(0,180,0),5,new Color(.2f,.3f,.4f));
        ...
        Sign("REARM  /  E",new Vector3(0,2.7f,-18.6f),Quaternion.identity,5,new Color(1,.65f,.2f));
        ...
        Sign("MEJORAS",new Vector3(0,1.45f,-17.45f),Quaternion.identity,2.8f,Color.white);
    ...
    {
        RenderSettings.ambientMode=AmbientMode.Flat;RenderSettings.ambientLight=new Color(.35f,.43f,.52f);RenderSettings.ambientIntensity=1;
        ...
        {
            var light=new GameObject("Ceiling wash").AddComponent<Light>();light.type=LightType.Point;light.transform.position=new Vector3(x,6,z);light.color=new Color(.4f,.75f,1);light.intensity=5;light.range=13;light.shadows=LightShadows.None;
        }
    ...
    static LabHUD CreateHUD(PlayerStateMachine player)
    {
        ...
        hud.weaponText=Label(go.transform,"Weapon",new Vector2(1,0),new Vector2(-360,132),new Vector2(320,40),22);
        hud.ammoText=Label(go.transform,"Ammo",new Vector2(1,0),new Vector2(-360,70),new Vector2(320,65),46);
