...
{
    static int CleanRenderer()
    {
        string path=Folder+"/LaboratoryRenderer.asset";
        ...
        if(renderer==null)
        {
            ...
            renderer.name="Laboratory Renderer";renderer.rendererFeatures.Clear();
        ...
        int index=list.arraySize;list.arraySize++;list.GetArrayElementAtIndex(index).objectReferenceValue=renderer;so.ApplyModifiedPropertiesWithoutUndo();
        return index;
    }
    ...
    static GameObject CreatePlayer()
    {
        ...
        if(ps.GetComponent<PlayerInteraction>()==null) ps.gameObject.AddComponent<PlayerInteraction>();
        ...
        var damageVolume=head.GetComponentsInChildren<UnityEngine.Rendering.Volume>(true).FirstOrDefault();
        if(damageVolume!=null)
        {
            var profile=Asset<UnityEngine.Rendering.VolumeProfile>("Damage feedback",()=>ScriptableObject.CreateInstance<UnityEngine.Rendering.VolumeProfile>());
            if(!profile.TryGet<Vignette>(out var vignette))vignette=profile.Add<Vignette>();
            vignette.intensity.Override(.25f);vignette.color.Override(new Color(.28f,.015f,.01f));vignette.smoothness.Override(.5f);
            if(!profile.TryGet<ChromaticAberration>(out var chromatic))chromatic=profile.Add<ChromaticAberration>();
            chromatic.intensity.Override(.015f);
            foreach(var component in profile.components) if(string.IsNullOrEmpty(AssetDatabase.GetAssetPath(component)))AssetDatabase.AddObjectToAsset(component,profile);
            damageVolume.sharedProfile=profile;
        }
        ...
        int rendererIndex=CleanRenderer();cd.SetRenderer(rendererIndex);wd.SetRenderer(rendererIndex);
        cd.renderPostProcessing=true;wd.renderPostProcessing=false;Set(wd,"m_ClearDepth",true);
        ...
        var mesh=Asset<Mesh>("Knife blade",()=>new Mesh());
        mesh.Clear();
        mesh.vertices=new[]{new Vector3(-.5f,-.5f,-.5f),new Vector3(.5f,-.5f,-.5f),new Vector3(.45f,.23f,-.5f),new Vector3(-.15f,.65f,0),new Vector3(-.5f,.25f,-.5f),new Vector3(-.5f,-.5f,.5f),new Vector3(.5f,-.5f,.5f),new Vector3(.45f,.23f,.5f),new Vector3(-.5f,.25f,.5f)};
        mesh.triangles=new[]{0,2,1,0,4,2,4,3,2,5,6,7,5,7,8,8,7,3,0,1,6,0,6,5,1,2,7,1,7,6,2,3,7,4,8,3,0,5,8,0,8,4};
        mesh.RecalculateNormals();mesh.RecalculateBounds();blade.GetComponent<MeshFilter>().sharedMesh=mesh;
