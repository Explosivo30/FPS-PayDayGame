...
public class OperatorHands : MonoBehaviour
{
    ...
    private Vector3[] upperHome=new Vector3[2];
    ...
    private void Awake()
    {
        ...
        for(int i=0;i<2;i++)
        {
            ...
            if(upper[i]!=null) { upperRest[i]=upper[i].localRotation;upperHome[i]=upper[i].localPosition; }
    ...
    private void LateUpdate()
    {
        ...
        for(int i=0;i<2;i++)
        {
            ...
            upper[i].localPosition=upperHome[i];upper[i].localRotation=upperRest[i]; fore[i].localRotation=foreRest[i];
    ...
    private static void Solve(Transform a,Transform b,Transform c,Vector3 goal,Vector3 hint)
    {
        ...
        Vector3 offset=goal-a.position;
        // Keep unreachable grips exact by moving the offscreen shoulder, never by stretching fingers.
        if(offset.magnitude>ab+bc-.01f) a.position+=offset.normalized*(offset.magnitude-(ab+bc-.01f));
