...
public abstract class BaseGun : MonoBehaviour, IWeapon, IReloadable, IBulletTracer
{
    ...
    private RecoilData runtimeRecoil;
    public virtual RecoilData Recoil
    {
        get { if (runtimeRecoil == null && recoilData != null) runtimeRecoil = Instantiate(recoilData); return runtimeRecoil; }
    }
    private WeaponAction action;
    public WeaponAction Action => action != null ? action : (action = GetComponent<WeaponAction>());
    ...
    {
        if (runtimeRecoil != null) Destroy(runtimeRecoil);
    ...
    {
        if (!Action.RequestFire()) return false;
        if (currentAmmo <= 0)
        {
            PlayDryFire();
            return false;
        ...
        lastShotTime = Mathf.Max(lastShotTime + secondsPerShot, Time.time - Time.deltaTime * .5f);
        currentAmmo--;
        Action.NotifyShot();
        return true;
    }
    ...
    /// </summary>
    protected bool FireHitscan(bool isAiming, Transform visualMuzzle, bool feedback = true)
    {
        Camera activeAimCamera = GetAimCamera();
        ...
        if (Physics.Linecast(aimTransform.position, muzzlePosition, out RaycastHit blocked, layerMask, QueryTriggerInteraction.Ignore))
        {
            HandleHit(blocked, damage, shotDirection);
            Play(muzzlePosition, blocked.point);
            if (feedback) PlayShotFeedback(muzzlePosition, shotDirection);
            return true;
        }
    ...
    public virtual void ApplyRecoil()
    {
        ...
        bool aiming = this is IAimable aimable && aimable.IsAiming;
        GunRecoil.EnsureExists().ApplyRecoil(Recoil, aiming ? .65f : 1f, 1f / Mathf.Max(.01f, fireRate));
    }

    protected Vector3 GetShootDirection(bool isAiming, Transform aimTransform)
    {
        float spreadDegrees = Recoil == null
            ? 0f
            : (isAiming ? Recoil.spreadADS : Recoil.spreadHip);
    ...
    public static AudioClip CreateProceduralShotClip(string weaponId, bool automatic)
    {
        const int sampleRate = 44100;
        bool plasma = weaponId == "Pistol";
        bool shotgun = weaponId == "ShotGun";
        float clipLength = shotgun ? .36f : automatic ? .14f : .22f;
        ...
        for (int i = 0; i < sampleCount; i++)
        {
            ...
            if (plasma) sample = sample * .65f + Mathf.Sin(2f * Mathf.PI * (450f * t - 650f * t * t)) * Mathf.Exp(-t * 22f) * .5f;
            if (shotgun) sample = sample * .8f + Mathf.Sin(2f * Mathf.PI * 58f * t) * Mathf.Exp(-t * 14f) * .55f;
            samples[i] = sample;
