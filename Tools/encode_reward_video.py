...
root=pathlib.Path("D:/Unity/FPS-PayDayGame/Artifacts/Rewards")
...
strip=editor.sequences.new_image("Reward cards",str(frames[0]),channel=1,frame_start=1)
...
if info["audioPeak"]>.0001:editor.sequences.new_sound("Game audio",str(root/"rewards-audio.wav"),channel=2,frame_start=1)
scene.frame_start=1;scene.frame_end=len(frames)
scene.render.resolution_x=1920;scene.render.resolution_y=1080;scene.render.resolution_percentage=100
scene.render.fps=30;scene.render.fps_base=seconds*30/len(frames)
...
scene.render.filepath=str(root/"Reward-cards.mp4")
scene.view_settings.view_transform="Standard";scene.view_settings.look="None";scene.render.use_sequencer=True
bpy.ops.render.render(animation=True)
print("REWARD VIDEO COMPLETE",len(frames),seconds)
